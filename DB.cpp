...
bool DB::loginUser(string username, string  password)
{
	...
	try
	{
		...
		{
			cout << "Query failed: " << mysql_error(conn) << endl;
			throw IException::Factory::err_Login();
		}
	}
	catch (IException* e)
	{
...
{
	// 6/Tanks
	string query = "SELECT username, score FROM Scor WHERE denumire_joc=\"" + denumire_joc + "\" ORDER BY score DESC limit 10;";
	const char* q = query.c_str();
	qstate = mysql_query(conn, q);  //executa query-ul
	try
	{
		if (!qstate)
		{
			...
			{
				raspuns += scoruri[i].first + ": " + scoruri[i].second + "/";
			}
			return raspuns;
		}
		else
...
{
	// 7
	string query = "SELECT username, denumire_joc, score FROM Scor ORDER BY score DESC limit 5;";
	const char* q = query.c_str();
	qstate = mysql_query(conn, q);  //executa query-ul
	try
	{
		if (!qstate)
		{
			res = mysql_store_result(conn);
			row = mysql_fetch_row(res);
			vector<tuple<string, string, string>> scoruri;
			while (row)
			{
				scoruri.push_back(make_tuple(row[0], row[1], row[2]));
				row = mysql_fetch_row(res);
			}
			string raspuns = " /";
			for (auto i : scoruri)
...
{
	// 11/Tanks
	string query = "SELECT gamepath FROM Jocuri WHERE denumire=\"" + denumire + "\";";
	const char* q = query.c_str();
	qstate = mysql_query(conn, q);  //executa query-ul
	try
	{
...
string DB::pathImage(string denumire)
{
	// 13/Tanks
	string query = "SELECT imagepath FROM Jocuri WHERE denumire=\"" + denumire + "\";";
	const char* q = query.c_str();
	qstate = mysql_query(conn, q);  //executa query-ul
	try
	{
		...
		{
			cout << "Query failed: " << mysql_error(conn) << endl;
			throw IException::Factory::err_PathImage();
		}
	}
	catch (IException* e)
	{
